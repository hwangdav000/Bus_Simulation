...
//  reads in config txt from command line and turns into routes_
void ConfigManager::ReadConfig(const std::string filename) {
  ...
  while (std::getline(configFile, line)) {
    ...
    if (chunk == "ROUTE_GENERAL") {
      ...
      //  Need to turn these lat and
      //  long into real-world distances
      //  This means moving 1 speed in a time
      //  click moves 1 mile. That's a bit far,
